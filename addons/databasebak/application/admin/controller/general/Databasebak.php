...
use addons\databasebak\library\Backup;
 ...
 */
class Databasebak extends Backend
{
    ...
    {
        $config = get_addon_config('databasebak');
        $backupDir = app()->getRootPath().'public'.DS.$config['backupDir'];
    ...
    public function restore($ids = '')
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($action == 'restore') {
                try {
                    $dir = app()->getRuntimePath().'database'.DS;
                    if (! is_dir($dir)) {
                        mkdir($dir, 0755);
                    }
    ...
    private function doquery($sql = null)
    {
        ...
        foreach ($sqls as $key => $val) {
            ...
            if (preg_match("/^(select|explain)(.*)/i ", $val)) {
                ...
                $time = number_format(($endTime - $beginTime), 5);
